...
import data.espece.faune.CanardLune;
import data.espece.faune.Chameau;
import data.espece.faune.Chat;
...
public class GestionnaireFactory {
	...
	public Animal createElement(Animals key, Map map) {
		switch (key) {
			...
			case CANARD_LUNE:
				return new CanardLune(0, 0, 0, null, null, null, null, map);
			case CHAT:
				return new Chat(0, 0, null, 0, null, null, null, null, map);
			case CHAMEAU:
				return new Chameau(0, 0, 0, null, null, null, null, map);
			case CHIEN:
	...
	public Structure createElement(Structures key, Map map) {
		switch (key) {
			...
			case CAGE_POISSON:
				return new CagePoisson(0, 0, null, map);
			case ENTREPOT:
			...
			case REFUGE_CANARDLUNE:
				return new RefugeCanardLune(0, 0, null, map);
			case REFUGE_CHAMEAU:
				return new RefugeChameau(0, 0, null, map);
